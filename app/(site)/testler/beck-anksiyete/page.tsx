...
// Yorumlama fonksiyonu (Beck Anksiyete)
function interpretBeckAnxiety(score: number) {
  if (score <= 7) return "Düşük düzeyde anksiyete"
...
export default function BeckAnxietyTestPage() {
  ...
  // contact info collected AFTER test completion
  ...
  // when user finishes the questions, compute result and show contact form
  const handleSubmit = () => {
    const total = answers.reduce((sum, a) => sum + a.value, 0)
    const interpretationText = interpretBeckAnxiety(total)
  ...
  // save results to Sanity after collecting contact info
