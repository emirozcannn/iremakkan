...
// 💬 Kısa Semptom Envanteri Yorumlama
function interpretBSI(score: number) {
  if (score <= 60) return 'Normal düzey'
  if (score <= 120) return 'Hafif düzeyde psikolojik belirti'
  if (score <= 180) return 'Orta düzeyde psikolojik belirti'
...
export default function KisaSemptomTestPage() {
  ...
  const handleSubmit = () => {
    ...
    const interpretationText = interpretBSI(total)
  ...
  const saveContactAndResult = async () => {
    ...
    try {
      ...
      console.log('✅ Test sonucu Sanity\'ye başarıyla kaydedildi:', data)
      setSubmitted(true)
