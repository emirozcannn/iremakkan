...
async function getServicesPageData(): Promise<ServicesPageData> {
  try {
    ...
    console.error("Services page data fetch error:", error);
    return {};
  }
}
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const pageTitle = pageData.seoTitle || "Psikolojik Danışmanlık Hizmetleri | İrem Akkan";
  const pageDescription = pageData.seoDescription || "Bireysel, çift ve aile danışmanlığı hizmetleri. Profesyonel psikolojik destek ile yaşam kalitenizi artırın. 8+ yıllık deneyim, %98 memnuniyet oranı.";
...
// Helper fonksiyon: ** işaretlerini span'a çevir
function parseTitle(title: string) {
  const parts = title.split('**');
  return parts.map((part, index) => 
    index % 2 === 1 ? (
      <span key={index} className="text-gradient-gold">{part}</span>
    ) : part
  );
}
...
export default async function ServicesPage() {
  ...
  const ctaButtonText = pageData.ctaButtonText || "Ücretsiz Ön Görüşme";
  const ctaButtonLink = pageData.ctaButtonLink || "/iletisim";
