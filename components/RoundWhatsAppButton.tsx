...
export default function RoundWhatsAppButton({
  ...
  size = 'md',
  className = ''
}: RoundWhatsAppButtonProps) {
  // WhatsApp URL'sini oluştur
  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`

  // Size stilleri - Tam yuvarlak
  const sizeStyles = {
    sm: 'w-12 h-12',
    md: 'w-14 h-14',
    lg: 'w-16 h-16'
  ...
  return (
    ...
    >
      {/* WhatsApp İkonu */}
      <svg 
        className={`${iconSizeStyles[size]} group-hover:scale-110 transition-transform duration-300`}
        fill="currentColor" 
        viewBox="0 0 24 24"
      >
      ...
      {/* Pulse efekti */}
      <div className="absolute inset-0 rounded-full bg-green-400 animate-ping opacity-20 group-hover:opacity-30"></div>
    </Link>
  )
}